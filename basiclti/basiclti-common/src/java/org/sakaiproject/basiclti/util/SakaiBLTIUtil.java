...
public class SakaiBLTIUtil {
	...
	public static Object handleGradebookLTI13(Site site,  Long tool_id, Map<String, Object> content, String userId,
		...
		try {
			...
			if (scoreGiven == null) {
				...
				gradingService.setAssignmentScoreString(gradebookColumn.getGradebookUid(), siteId, gradebookColumn.getId(), userId, assignedGrade.toString(), "External Outcome");
				if ( StringUtils.isBlank(comment) ) {
					gradingService.deleteAssignmentScoreComment(gradebookColumn.getGradebookUid(), gradebookColumn.getId(), userId);
				} else {
					gradingService.setAssignmentScoreComment(gradebookColumn.getGradebookUid(), gradebookColumn.getId(), userId, comment);
	...
	public static org.sakaiproject.grading.api.Assignment getGradebookColumn(Site site, String userId, String title, SakaiLineItem lineItem) {
		...
		try {
			...
			// S2U-26 even if a user can potentially have multiple gradebooks, for LTI we're only allowing one return column
			if (gradingService.isGradebookGroupEnabled(siteId)) {
				userGradebooks = gradingService.getGradebookInstancesForUser(siteId, userId);
				// S2U-26 we select the first as default in case we have to add a new one
		...
		if (returnColumn == null) {
			...
			try {
				...
				//TODO S2U-26 para todas las instancias? caso nuevo add
