...
public class DateManagerServiceImpl implements DateManagerService {
	...
	public DateManagerValidation validateGradebookItems(String siteId, JSONArray gradebookItems) throws Exception {
		...
		for (int i = 0; i < gradebookItems.size(); i++) {
			...
			try {				
				Long itemId;
				if (jsonItem.get("id").getClass().getName().indexOf("Long") != -1) {
					itemId = (Long)jsonItem.get("id");
				} else {
					itemId = Long.parseLong((String)jsonItem.get("id"));
				}
				if (itemId == null) {
	...
	public DateManagerValidation validateSignupMeetings(String siteId, JSONArray signupMeetings) throws Exception {
		...
		for (int i = 0; i < signupMeetings.size(); i++) {
			...
			try {
				...
				Long meetingId;
				if (jsonMeeting.get("id").getClass().getName().indexOf("Long") != -1) {
					meetingId = (Long)jsonMeeting.get("id");
				} else {
					meetingId = Long.parseLong((String)jsonMeeting.get("id"));
				}
				if (meetingId == null) {
	...
	public DateManagerValidation validateForums(String siteId, JSONArray forums) throws Exception {
		...
		for (int i = 0; i < forums.size(); i++) {
			...
			try {
				...
				Long forumId;
				if (jsonForum.get("id").getClass().getName().indexOf("Long") != -1) {
					forumId = (Long)jsonForum.get("id");
				} else {
					forumId = Long.parseLong((String)jsonForum.get("id"));
				}
				if (forumId == null) {
	...
	public void updateTool(String toolId, DateManagerValidation dateManagerValidation) {
		try {
			...
			if (DateManagerConstants.COMMON_ID_ASSESSMENTS.equals(toolId)) {
				this.updateAssessments(dateManagerValidation);
			}
			if (DateManagerConstants.COMMON_ID_GRADEBOOK.equals(toolId)) {
				this.updateGradebookItems(dateManagerValidation);
			}
			if (DateManagerConstants.COMMON_ID_SIGNUP.equals(toolId)) {
				this.updateSignupMeetings(dateManagerValidation);
			}
			if (DateManagerConstants.COMMON_ID_RESOURCES.equals(toolId)) {
				this.updateResources(dateManagerValidation);
			}
			if (DateManagerConstants.COMMON_ID_CALENDAR.equals(toolId)) {
				this.updateCalendarEvents(dateManagerValidation);
			}
			if (DateManagerConstants.COMMON_ID_FORUMS.equals(toolId)) {
				this.updateForums(dateManagerValidation);
			}
			if (DateManagerConstants.COMMON_ID_ANNOUNCEMENTS.equals(toolId)) {
				this.updateAnnouncements(dateManagerValidation);
			}
			if (DateManagerConstants.COMMON_ID_LESSONS.equals(toolId)) {
				this.updateLessons(dateManagerValidation);
			}
		} catch (Exception ex) {
			log.error("Cannot update the tool receibed", ex); 
		}
	}
	...
	public boolean isChanged(String toolId, String[] columns) {
		...
		if (DateManagerConstants.COMMON_ID_GRADEBOOK.equals(toolId.replaceAll("\"", ""))) {
			org.sakaiproject.service.gradebook.shared.Assignment gbitem = gradebookService.getAssignmentByID(Long.parseLong(id));
	...
	public boolean compareDates(Date date, String dateString) {
		...
		if (dateString != null && !StringUtils.isBlank(dateString.replaceAll("\"", ""))) {
			if (date != null) {
